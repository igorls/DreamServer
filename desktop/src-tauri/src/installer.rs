...
pub fn run_install(
    ...
    // Drain both stdout and stderr in separate threads to prevent pipe buffer deadlock.
    // If either pipe fills its OS buffer (~64KB) without being read, the child process blocks.
            ...
            for line in reader.lines() {
                if let Ok(line) = line {
                    if let Some(progress) = parse_progress_line(&line) {
                        update_progress(&state_for_stdout, &progress.message, progress.percent);
                    }
                }
            }
        }
    ...
    let stderr_thread = std::thread::spawn(move || {
        if let Some(stderr) = stderr {
            ...
            for line in reader.lines() {
                if let Ok(line) = line {
                    eprintln!("[INSTALLER STDERR] {}", line);
                }
            }
        }
    });
