...
*   [Tony363](https://github.com/Tony363) — Hardened service-registry.sh against shell injection in service IDs, narrowed silent exception catches in db.py to specific types, improved PII scrubber with Luhn check for credit card detection and deterministic token round-trip, fixed token-spy settings persistence with atomic writes replacing broken fcntl locking, fixed SSH command injection in session-manager.sh using stdin piping with shlex.quote, narrowed broad exception catches across dashboard-api to specific types with appropriate log levels, added test coverage infrastructure with stale path fixes, and added CLAUDE.md with project instructions and design philosophy
