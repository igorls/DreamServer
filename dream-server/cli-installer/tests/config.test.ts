...
describe('config.ts', () => {
  ...
  beforeEach(() => {
    ...
    // Order: WebSearch, ImageGen, Voice, Workflows, RAG, LiteLLM, OpenClaw, DevTools
    multiSelectSpy = spyOn(prompts, 'multiSelect').mockImplementation(async () => [true, true, true, true, true, false, false, false]);
  ...
  test('config() updates features and restarts services', async () => {
    ...
    // Choose features — order: WebSearch, ImageGen, Voice, Workflows, RAG, LiteLLM, OpenClaw, DevTools
  ...
  test('config() handles no changes correctly', async () => {
    // Seed .env with all 8 features matching the mock return
    fs.writeFileSync(join(tmpDir, '.env'), [
      'ENABLE_WEB_SEARCH_STACK=true',
    ...
    // Return same values as current .env — order: WebSearch, ImageGen, Voice, Workflows, RAG, LiteLLM, OpenClaw, DevTools
    multiSelectSpy.mockImplementation(async () => [true, true, true, true, true, false, false, false]);
